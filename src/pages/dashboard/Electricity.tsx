...
export default function Electricity() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      verifyMutation.mutate(
        ...
        {
          onSuccess: (res) =>
            setCustomerName(res?.content?.Customer_Name || ""),
  ...
  const canSubmit =
    !!service && meterValid && !!customerName && finalAmount >= 500;
  ...
  const openPin = () => {
    ...
    if (!canSubmit) {
      setError(
        "Select a provider, enter a valid meter number, and an amount of at least ₦500.",
      );
      return;
    }
    setPinOpen(true);
  ...
  return (
    <div className="mx-auto max-w-xl">
      <PageHeader
        icon={Zap}
        title="Electricity"
        subtitle="Prepaid and postpaid, all major DisCos."
      />
      ...
      <div className="card space-y-6 p-6 sm:p-7">
          ...
          {servicesLoading ? (
            <div className="grid grid-cols-3 gap-2.5">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="h-16 animate-pulse rounded-xl bg-cream-100"
                />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-2.5">
              {(services ?? []).map((s: VTPassService) => (
                <button
                  ...
                  className={`relative rounded-xl border px-2.5 py-3.5 text-center text-[11px] font-semibold leading-tight transition-all duration-200 ${
                    service?.serviceID === s.serviceID
                      ? "border-forest-800 bg-forest-900/5 text-forest-950 shadow-sm"
                      : "border-line text-ink-700 hover:-translate-y-0.5 hover:border-forest-800/30 hover:shadow-sm"
                  }`}
                >
                  {service?.serviceID === s.serviceID && (
        ...
        <label className="block">
          ...
          {customerName && (
            <p className="mt-1.5 text-xs font-medium text-leaf-600">
              {customerName}
            </p>
          )}
          {!customerName &&
          ...
          <div className="grid grid-cols-5 gap-2">
            {presets.map((p) => (
              <button
                ...
                className={`rounded-xl border px-1 py-2.5 font-mono text-[11px] font-semibold transition-all duration-200 ${
                  amount === p
